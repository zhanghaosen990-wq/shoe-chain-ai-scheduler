...
(async () => {
 ...
 try {
  ...
  await page.goto(base);await page.getByRole('button',{name:/寻找制造伙伴/}).waitFor();
  ...
  await page.getByRole('button',{name:'保存产能与工艺',exact:true}).click();
  await page.getByRole('status').filter({hasText:'产能与工艺已保存'}).waitFor();
  await page.getByRole('button',{name:'工厂档案与展示',exact:true}).click();
  ...
  await page.getByRole('button',{name:'保存工厂档案',exact:true}).click();
  await page.getByRole('status').filter({hasText:'工厂档案已保存'}).waitFor();
