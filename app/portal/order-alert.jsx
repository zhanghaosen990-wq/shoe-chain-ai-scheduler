...
export function OrderAlerts({data,account,mutate,refresh,notify,Modal}){
 ...
 const [open,setOpen]=useState(false),[target,setTarget]=useState(null);
 const seen=useRef(new Set()),previousPath=useRef(null),openRef=useRef(false);
 const signature=pending.map(o=>o.id).sort().join('|');
 function show(){pending.forEach(o=>seen.current.add(o.id));openRef.current=true;setOpen(true);}
 ...
 useEffect(()=>{
  if(!target||location.pathname!=='/factory')return;
  const frame=requestAnimationFrame(()=>{const row=document.getElementById('factory-order-'+target.id);if(row){row.focus({preventScroll:true});row.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'center'});setTarget(null);}});
  return()=>cancelAnimationFrame(frame);
 },[target,location.pathname]);
 function details(order){previousPath.current='/factory';close();setTarget({id:order.id});navigate('/factory');}
 return <><button className="notification-bell" aria-label={`订单通知，${pending.length} 条待处理`} title="待接单通知" onClick={show}><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9ZM10 21h4"/></svg>{pending.length>0&&<span className="notification-count">{pending.length>99?'99+':pending.length}</span>}</button>{open&&<Modal title="🔔 收到新的品牌方合作邀约" onClose={close}><div className="order-alert-intro"><strong>待接单通知</strong><span>{pending.length} 笔需求等待你处理</span></div>{pending.length?<div className="order-alert-list">{pending.map(order=><AlertCard key={order.id} order={order} data={data} account={account} mutate={mutate} refresh={refresh} notify={notify} onHandled={close} onDetails={()=>details(order)}/>)}</div>:<div className="empty">暂无待处理订单，新的合作邀约会在这里提醒。</div>}</Modal>}</>;
}
