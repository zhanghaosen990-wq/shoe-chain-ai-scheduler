...
function createAuth(storage,cryptoApi=globalThis.crypto){
 ...
 async function initialize(){
  for(const [id,username,name,role] of seeds){
   ...
   const salt=randomId(cryptoApi),passwordHash=await hash('123456',salt,cryptoApi);
 ...
 async function register(input){
  ...
  const salt=randomId(cryptoApi),passwordHash=await hash(input.password,salt,cryptoApi);
  const users=read();if(users.some(u=>u.username.toLowerCase()===username.toLowerCase()))throw Error('该账号已注册，请直接登录');
  const user={id:randomId(cryptoApi),profileReady:true,username,name,role:input.role,salt,passwordHash,contactName:'',phone:'',description:'',categories:[]};
 ...
 async function login(username,password){
  ...
  if(!user||await hash(password,user.salt,cryptoApi)!==user.passwordHash)throw Error('账号或密码不正确');
